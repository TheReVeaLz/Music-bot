   ...
   async execute({ inter }) {
        ...
        switch (inter.options._hoistedOptions.map(x => x.value).toString()) {
            case 'enable_loop_queue': {
                if (queue.repeatMode === QueueRepeatMode.TRACK) return inter.reply({ content: `You must first disable the current music in the loop mode (\`/loop Disable\`) ${inter.member}... try again ? ❌` });
            ...
            case 'disable_loop': {
                if (queue.repeatMode === QueueRepeatMode.OFF) return inter.reply({ content: await Translate(`You must first enable the loop mode <(/loop Queue or /loop Song)> <${inter.member}>... try again ? <❌>`) });
            ...
            case 'enable_loop_song': {
                if (queue.repeatMode === QueueRepeatMode.QUEUE) return inter.reply({ content: await Translate(`You must first disable the current music in the loop mode <(\`/loop Disable\`)> <${inter.member}>... try again ? <❌>`) });
            ...
            case 'enable_autoplay': {
                if (queue.repeatMode === QueueRepeatMode.AUTOPLAY) return inter.reply({ content: await Translate(`You must first disable the current music in the loop mode <(\`/loop Disable\`)> <${inter.member}>... try again ? <❌>`) });
